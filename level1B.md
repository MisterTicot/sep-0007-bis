...
In CosmicLink, I use an arbitrary sequence/account number along with the
injunction (`&stripSource`, `&stripSequence`) that wallet must replace it with
user values. However, this is weird and bloated.

You'll have to pass account & sequence numbers and, if it have to be replaced,
it is more meaningful to pass neutral values than arbitrary ones. It also save
about 20-30 characters which leave space for another operation in a GET request.
